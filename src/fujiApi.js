...
const json2csv_2 = require("json2csv");
...
function loggerFormat() {
    if (process.env.SEARCHKIT_USE_JSON_LOGGING === 'true') {
        ...
        return winston_1.default.format.printf(({ level, message, timestamp }) => `[${timestamp}][${level}] ${message}`);
    }
}
// Logger
const logger = winston_1.default.createLogger({
    level: logLevel,
    format: winston_1.default.format.combine(winston_1.default.format.timestamp(), winston_1.default.format.splat(), loggerFormat()),
    transports: [
...
const client = elasticsearchUsername && elasticsearchPassword ? new elasticsearch_1.Client({
...
const storage = new storage_1.Storage(); //localhost test auth
const bucketName = 'cessda-fuji-storage-dev';
function fujiMetrics() {
    (async () => {
        const cdcLinks = new sitemapper_1.default({
        ...
        try {
            ...
            const input = new stream_1.Readable({ objectMode: true }); //initiating CSV writer
            input._read = () => { };
            //const arrayTests = sites.slice(0, 5); //DEBUG CODE FOR TESTS - REDUCE ARRAY TO 5 STUDIES!!!!!!!!!!!!!!!!!!
            for (const site of sites) {
                await apiLoop(site, fullDate).then(data => input.push(data));
            }
            input.push(null);
            const outputLocal = (0, fs_1.createWriteStream)(`../runResults/CSV_DATA_${fullDate}.csv`, { encoding: 'utf8' });
            const fields = [
            ...
            (0, json2csv_2.parseAsync)(processor, opts).then(csv => logger.info('CSV file created succesfully')).catch(err => logger.error(`Error at CSV writer: ${err}`));
        }
        catch (error) {
...
async function apiLoop(link, fullDate) {
    const urlLink = new url_1.URL(link);
    ...
    const response = await (0, node_fetch_1.default)(cdcApiUrl);
...
async function resultsToElastic(fileName, fujiResults) {
    try {
        const elasticdoc = {
            index: 'fuji-results',
            id: fileName,
...
async function uploadFromMemory(fileName, fujiResults) {
    /* DEBUG CODE
    ...
    */
    ...
    logger.info(`${fileName} with contents uploaded to ${bucketName}.`);
}
function resultsToHDD(fileName, fujiResults) {
    fs_1.default.writeFile(`../runResults/${fileName}`, JSON.stringify(fujiResults, null, 4).toString(), (err) => {
        if (err)
